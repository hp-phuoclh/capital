...
<div class="row mb-2">
  ...
  <div class="col-sm-6">
    <ol class="breadcrumb float-sm-right">
      ...
      <li class="breadcrumb-item">{{__('Edit order')}}</li>
    </ol>
  </div>
</div>
@endsection
...
<form action="{{route('orders.update', ['order' => $order->id])}}" method="POST" id="create_order_form">
  @csrf
  @method('PUT')
              ...
              <div class="col-sm-12">
                ...
                <div class="form-group">
                    <div class="wrap_address">
                        <input type="text" name="address" id="address" class="form-control{{ $errors->has('address') ? ' is-invalid' : '' }}" value="{{ old('address',$order->address) }}" placeholder="{{__('Input Address')}} ">
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped table-valign-middle table-default">
                            ...
                            <tbody id="list_product_order">
                                ...
                                <tr data-id="{{$item->product->id}}" id="pr_{{$item->product->id}}">
                                    <td>{{$item->product->id}}</td>
                                    <td>{{$item->product->code}}</td>
                                    <td><div class="img"><img class="img-fit img-fit-sm" src="/uploads/products/1/143ab6ff012497c0b7db93096273f48d1592815010.jpeg" alt=""></div></td>
                                    <td>Ca phe den</td>
                                    <td>
                                        <div><s>{{$item->product->sale_off ? number_format($item->product->price,0,',','.') : ''}}</s></div>
                                        <div>{{$item->product->sale_off ? number_format($item->product->sale_off,0,',','.') : number_format($item->product->price,0,',','.')}}</div>
                                        <input type="hidden" class="product_price" value="{{$item->product->sale_off ? $item->product->sale_off : $item->product->price}}">
                                    </td>
                                    <td>
                                        <input type="number" value="{{$item->quantity}}" min="1" step="1" class="quantity_product" name="products[{{$item->product->id}}]">
                                    </td>
      ...
      <div class="card">
        <div class="card-body">
                <div class="row">
                  <div class="col-md-12">
                      <div class="form-group">
                          <textarea name="note" class="form-control{{ $errors->has('note') ? ' is-invalid' : '' }}" id="note" placeholder="{{__('Note')}}" cols="30" rows="4" style="height: 125px">{{ old('note',$order->note) }}</textarea>
                              @if ($errors->has('note'))
                                  <span class="invalid-feedback" role="alert">
...
<script>
  ...
  var $option_user = '';
  @if($order->user)
    $option_user = $("<option selected></option>").val("{{$order->user_id}}").text("{{$order->user->name.' - '.$order->user->phone.' - '.$order->user->email}}");
  @endif
  $('#search_user').select2({
        ajax: {
            type: 'GET',
        ...
        theme: 'bootstrap4',
        width: '100%',
    }).append($option_user).trigger('change');

    // search store
  var $option_store = '';
  @if($order->store)
    $option_store = $("<option selected></option>").val("{{$order->store_id}}").text("{{$order->store->name.' - '.$order->store->phone.' - '.$order->store->email.' - '.$order->store->address}}");
  @endif
  $('#search_store').select2({
        ...
        theme: 'bootstrap4',
        width: '100%',
    }).append($option_store).trigger('change');
       ...
       $.ajax({
            ...
            success: function (result) {
                if(result.status == 'success') {
                    ...
                    toasts("danger",result.message,"{{__('Notifications')}}");
                }
            },
